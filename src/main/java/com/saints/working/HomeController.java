...
public class HomeController {
	...
	public String confirm(Model model, HttpServletRequest request) throws TwitterException {
		...
		// sets up twitter for sending tweet, calling on the props class for the
		// information
		...
		// if they have no twitter, tweets to the company, if they have twitter,
		// tweets to the companys twitter name
	...
	public String CompanyDonations(Model model, HttpServletRequest request) {
		...
		// gets list of items for pickup which contain company profile and
		...
		// loops thru all items and removes any whose ID does not equal company
		// ID
		Iterator<itemsForPickup> iter = items.iterator();
		while (iter.hasNext()) {
	...
	public String adminRecentDonations(Model model, HttpServletRequest request) {
		...
		// loops thru all donations and removes any that are not complete or are
		// before 1 month ago.
		Iterator<itemsForPickup> iter = items.iterator();
		while (iter.hasNext()) {
			itemsForPickup tempItem = iter.next();
			...
			// removes any donations that are not complete or are before 1 month
			// ago.
	...
	public String removeFromCompanyDonationPage(Model model, HttpServletRequest request) {
		// deletes donation based off companyID from hidden form on
		// corresponding page
